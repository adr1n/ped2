-- Copyright 1986-2022 Xilinx, Inc. All Rights Reserved.
-- Copyright 2022-2023 Advanced Micro Devices, Inc. All Rights Reserved.
-- --------------------------------------------------------------------------------
-- Tool Version: Vivado v.2023.1 (win64) Build 3865809 Sun May  7 15:05:29 MDT 2023
-- Date        : Wed Oct  4 11:59:09 2023
-- Host        : Oct running 64-bit major release  (build 9200)
-- Command     : write_vhdl -mode funcsim -nolib -force -file {C:/Users/oabin/Projetos
--               Vivado/Ped2/Ped2_Exp1_FSMs/Ped2_Exp1_FSMs.sim/sim_1/impl/func/xsim/Top_module_func_impl.vhd}
-- Design      : Top_module
-- Purpose     : This VHDL netlist is a functional simulation representation of the design and should not be modified or
--               synthesized. This netlist cannot be used for SDF annotated simulation.
-- Device      : xc7a35tcpg236-1
-- --------------------------------------------------------------------------------
library IEEE;
use IEEE.STD_LOGIC_1164.ALL;
library UNISIM;
use UNISIM.VCOMPONENTS.ALL;
entity Clock05 is
  port (
    CLK : out STD_LOGIC;
    clockdiv_reg_0 : in STD_LOGIC;
    AR : in STD_LOGIC_VECTOR ( 0 to 0 )
  );
end Clock05;

architecture STRUCTURE of Clock05 is
  signal \^clk\ : STD_LOGIC;
  signal clockdiv_i_1_n_0 : STD_LOGIC;
  signal counter : STD_LOGIC_VECTOR ( 27 downto 0 );
  signal \counter0_carry__0_n_0\ : STD_LOGIC;
  signal \counter0_carry__1_n_0\ : STD_LOGIC;
  signal \counter0_carry__2_n_0\ : STD_LOGIC;
  signal \counter0_carry__3_n_0\ : STD_LOGIC;
  signal \counter0_carry__4_n_0\ : STD_LOGIC;
  signal counter0_carry_n_0 : STD_LOGIC;
  signal \counter[27]_i_2_n_0\ : STD_LOGIC;
  signal \counter[27]_i_3_n_0\ : STD_LOGIC;
  signal \counter[27]_i_4_n_0\ : STD_LOGIC;
  signal \counter[27]_i_5_n_0\ : STD_LOGIC;
  signal \counter[27]_i_6_n_0\ : STD_LOGIC;
  signal \counter[27]_i_7_n_0\ : STD_LOGIC;
  signal \counter[27]_i_8_n_0\ : STD_LOGIC;
  signal counter_0 : STD_LOGIC_VECTOR ( 27 downto 0 );
  signal data0 : STD_LOGIC_VECTOR ( 27 downto 1 );
  signal NLW_counter0_carry_CO_UNCONNECTED : STD_LOGIC_VECTOR ( 2 downto 0 );
  signal \NLW_counter0_carry__0_CO_UNCONNECTED\ : STD_LOGIC_VECTOR ( 2 downto 0 );
  signal \NLW_counter0_carry__1_CO_UNCONNECTED\ : STD_LOGIC_VECTOR ( 2 downto 0 );
  signal \NLW_counter0_carry__2_CO_UNCONNECTED\ : STD_LOGIC_VECTOR ( 2 downto 0 );
  signal \NLW_counter0_carry__3_CO_UNCONNECTED\ : STD_LOGIC_VECTOR ( 2 downto 0 );
  signal \NLW_counter0_carry__4_CO_UNCONNECTED\ : STD_LOGIC_VECTOR ( 2 downto 0 );
  signal \NLW_counter0_carry__5_CO_UNCONNECTED\ : STD_LOGIC_VECTOR ( 3 downto 0 );
  signal \NLW_counter0_carry__5_O_UNCONNECTED\ : STD_LOGIC_VECTOR ( 3 to 3 );
  attribute ADDER_THRESHOLD : integer;
  attribute ADDER_THRESHOLD of counter0_carry : label is 35;
  attribute ADDER_THRESHOLD of \counter0_carry__0\ : label is 35;
  attribute ADDER_THRESHOLD of \counter0_carry__1\ : label is 35;
  attribute ADDER_THRESHOLD of \counter0_carry__2\ : label is 35;
  attribute ADDER_THRESHOLD of \counter0_carry__3\ : label is 35;
  attribute ADDER_THRESHOLD of \counter0_carry__4\ : label is 35;
  attribute ADDER_THRESHOLD of \counter0_carry__5\ : label is 35;
  attribute SOFT_HLUTNM : string;
  attribute SOFT_HLUTNM of \counter[0]_i_1\ : label is "soft_lutpair2";
  attribute SOFT_HLUTNM of \counter[26]_i_1\ : label is "soft_lutpair2";
begin
  CLK <= \^clk\;
clockdiv_i_1: unisim.vcomponents.LUT6
    generic map(
      INIT => X"FFFFFFFE00000001"
    )
        port map (
      I0 => AR(0),
      I1 => \counter[27]_i_4_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_2_n_0\,
      I4 => counter(0),
      I5 => \^clk\,
      O => clockdiv_i_1_n_0
    );
clockdiv_reg: unisim.vcomponents.FDRE
    generic map(
      INIT => '1'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      D => clockdiv_i_1_n_0,
      Q => \^clk\,
      R => '0'
    );
counter0_carry: unisim.vcomponents.CARRY4
     port map (
      CI => '0',
      CO(3) => counter0_carry_n_0,
      CO(2 downto 0) => NLW_counter0_carry_CO_UNCONNECTED(2 downto 0),
      CYINIT => counter(0),
      DI(3 downto 0) => B"0000",
      O(3 downto 0) => data0(4 downto 1),
      S(3 downto 0) => counter(4 downto 1)
    );
\counter0_carry__0\: unisim.vcomponents.CARRY4
     port map (
      CI => counter0_carry_n_0,
      CO(3) => \counter0_carry__0_n_0\,
      CO(2 downto 0) => \NLW_counter0_carry__0_CO_UNCONNECTED\(2 downto 0),
      CYINIT => '0',
      DI(3 downto 0) => B"0000",
      O(3 downto 0) => data0(8 downto 5),
      S(3 downto 0) => counter(8 downto 5)
    );
\counter0_carry__1\: unisim.vcomponents.CARRY4
     port map (
      CI => \counter0_carry__0_n_0\,
      CO(3) => \counter0_carry__1_n_0\,
      CO(2 downto 0) => \NLW_counter0_carry__1_CO_UNCONNECTED\(2 downto 0),
      CYINIT => '0',
      DI(3 downto 0) => B"0000",
      O(3 downto 0) => data0(12 downto 9),
      S(3 downto 0) => counter(12 downto 9)
    );
\counter0_carry__2\: unisim.vcomponents.CARRY4
     port map (
      CI => \counter0_carry__1_n_0\,
      CO(3) => \counter0_carry__2_n_0\,
      CO(2 downto 0) => \NLW_counter0_carry__2_CO_UNCONNECTED\(2 downto 0),
      CYINIT => '0',
      DI(3 downto 0) => B"0000",
      O(3 downto 0) => data0(16 downto 13),
      S(3 downto 0) => counter(16 downto 13)
    );
\counter0_carry__3\: unisim.vcomponents.CARRY4
     port map (
      CI => \counter0_carry__2_n_0\,
      CO(3) => \counter0_carry__3_n_0\,
      CO(2 downto 0) => \NLW_counter0_carry__3_CO_UNCONNECTED\(2 downto 0),
      CYINIT => '0',
      DI(3 downto 0) => B"0000",
      O(3 downto 0) => data0(20 downto 17),
      S(3 downto 0) => counter(20 downto 17)
    );
\counter0_carry__4\: unisim.vcomponents.CARRY4
     port map (
      CI => \counter0_carry__3_n_0\,
      CO(3) => \counter0_carry__4_n_0\,
      CO(2 downto 0) => \NLW_counter0_carry__4_CO_UNCONNECTED\(2 downto 0),
      CYINIT => '0',
      DI(3 downto 0) => B"0000",
      O(3 downto 0) => data0(24 downto 21),
      S(3 downto 0) => counter(24 downto 21)
    );
\counter0_carry__5\: unisim.vcomponents.CARRY4
     port map (
      CI => \counter0_carry__4_n_0\,
      CO(3 downto 0) => \NLW_counter0_carry__5_CO_UNCONNECTED\(3 downto 0),
      CYINIT => '0',
      DI(3 downto 0) => B"0000",
      O(3) => \NLW_counter0_carry__5_O_UNCONNECTED\(3),
      O(2 downto 0) => data0(27 downto 25),
      S(3) => '0',
      S(2 downto 0) => counter(27 downto 25)
    );
\counter[0]_i_1\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"00FE"
    )
        port map (
      I0 => \counter[27]_i_4_n_0\,
      I1 => \counter[27]_i_3_n_0\,
      I2 => \counter[27]_i_2_n_0\,
      I3 => counter(0),
      O => counter_0(0)
    );
\counter[10]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(10),
      O => counter_0(10)
    );
\counter[11]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(11),
      O => counter_0(11)
    );
\counter[12]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(12),
      O => counter_0(12)
    );
\counter[13]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(13),
      O => counter_0(13)
    );
\counter[14]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(14),
      O => counter_0(14)
    );
\counter[15]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(15),
      O => counter_0(15)
    );
\counter[16]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(16),
      O => counter_0(16)
    );
\counter[17]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(17),
      O => counter_0(17)
    );
\counter[18]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(18),
      O => counter_0(18)
    );
\counter[19]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(19),
      O => counter_0(19)
    );
\counter[1]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(1),
      O => counter_0(1)
    );
\counter[20]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(20),
      O => counter_0(20)
    );
\counter[21]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(21),
      O => counter_0(21)
    );
\counter[22]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(22),
      O => counter_0(22)
    );
\counter[23]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(23),
      O => counter_0(23)
    );
\counter[24]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(24),
      O => counter_0(24)
    );
\counter[25]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(25),
      O => counter_0(25)
    );
\counter[26]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(26),
      O => counter_0(26)
    );
\counter[27]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(27),
      O => counter_0(27)
    );
\counter[27]_i_2\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFF7FFF"
    )
        port map (
      I0 => counter(16),
      I1 => counter(17),
      I2 => counter(14),
      I3 => counter(15),
      I4 => \counter[27]_i_5_n_0\,
      O => \counter[27]_i_2_n_0\
    );
\counter[27]_i_3\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFFFFFB"
    )
        port map (
      I0 => counter(8),
      I1 => counter(9),
      I2 => counter(6),
      I3 => counter(7),
      I4 => \counter[27]_i_6_n_0\,
      O => \counter[27]_i_3_n_0\
    );
\counter[27]_i_4\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFFFFFD"
    )
        port map (
      I0 => counter(27),
      I1 => counter(26),
      I2 => counter(1),
      I3 => \counter[27]_i_7_n_0\,
      I4 => \counter[27]_i_8_n_0\,
      O => \counter[27]_i_4_n_0\
    );
\counter[27]_i_5\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"FFDF"
    )
        port map (
      I0 => counter(19),
      I1 => counter(18),
      I2 => counter(21),
      I3 => counter(20),
      O => \counter[27]_i_5_n_0\
    );
\counter[27]_i_6\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"FFFE"
    )
        port map (
      I0 => counter(11),
      I1 => counter(10),
      I2 => counter(13),
      I3 => counter(12),
      O => \counter[27]_i_6_n_0\
    );
\counter[27]_i_7\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"7FFF"
    )
        port map (
      I0 => counter(23),
      I1 => counter(22),
      I2 => counter(25),
      I3 => counter(24),
      O => \counter[27]_i_7_n_0\
    );
\counter[27]_i_8\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"FFFE"
    )
        port map (
      I0 => counter(3),
      I1 => counter(2),
      I2 => counter(5),
      I3 => counter(4),
      O => \counter[27]_i_8_n_0\
    );
\counter[2]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(2),
      O => counter_0(2)
    );
\counter[3]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(3),
      O => counter_0(3)
    );
\counter[4]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(4),
      O => counter_0(4)
    );
\counter[5]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(5),
      O => counter_0(5)
    );
\counter[6]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(6),
      O => counter_0(6)
    );
\counter[7]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(7),
      O => counter_0(7)
    );
\counter[8]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(8),
      O => counter_0(8)
    );
\counter[9]_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFE0000"
    )
        port map (
      I0 => counter(0),
      I1 => \counter[27]_i_2_n_0\,
      I2 => \counter[27]_i_3_n_0\,
      I3 => \counter[27]_i_4_n_0\,
      I4 => data0(9),
      O => counter_0(9)
    );
\counter_reg[0]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(0),
      Q => counter(0)
    );
\counter_reg[10]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(10),
      Q => counter(10)
    );
\counter_reg[11]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(11),
      Q => counter(11)
    );
\counter_reg[12]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(12),
      Q => counter(12)
    );
\counter_reg[13]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(13),
      Q => counter(13)
    );
\counter_reg[14]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(14),
      Q => counter(14)
    );
\counter_reg[15]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(15),
      Q => counter(15)
    );
\counter_reg[16]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(16),
      Q => counter(16)
    );
\counter_reg[17]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(17),
      Q => counter(17)
    );
\counter_reg[18]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(18),
      Q => counter(18)
    );
\counter_reg[19]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(19),
      Q => counter(19)
    );
\counter_reg[1]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(1),
      Q => counter(1)
    );
\counter_reg[20]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(20),
      Q => counter(20)
    );
\counter_reg[21]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(21),
      Q => counter(21)
    );
\counter_reg[22]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(22),
      Q => counter(22)
    );
\counter_reg[23]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(23),
      Q => counter(23)
    );
\counter_reg[24]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(24),
      Q => counter(24)
    );
\counter_reg[25]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(25),
      Q => counter(25)
    );
\counter_reg[26]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(26),
      Q => counter(26)
    );
\counter_reg[27]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(27),
      Q => counter(27)
    );
\counter_reg[2]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(2),
      Q => counter(2)
    );
\counter_reg[3]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(3),
      Q => counter(3)
    );
\counter_reg[4]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(4),
      Q => counter(4)
    );
\counter_reg[5]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(5),
      Q => counter(5)
    );
\counter_reg[6]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(6),
      Q => counter(6)
    );
\counter_reg[7]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(7),
      Q => counter(7)
    );
\counter_reg[8]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(8),
      Q => counter(8)
    );
\counter_reg[9]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => clockdiv_reg_0,
      CE => '1',
      CLR => AR(0),
      D => counter_0(9),
      Q => counter(9)
    );
end STRUCTURE;
library IEEE;
use IEEE.STD_LOGIC_1164.ALL;
library UNISIM;
use UNISIM.VCOMPONENTS.ALL;
entity FSM1_Matricula_Otavio is
  port (
    S0_OBUF : out STD_LOGIC;
    CLK : in STD_LOGIC;
    AR : in STD_LOGIC_VECTOR ( 0 to 0 );
    Dado0_IBUF : in STD_LOGIC
  );
end FSM1_Matricula_Otavio;

architecture STRUCTURE of FSM1_Matricula_Otavio is
  signal estado_atual : STD_LOGIC_VECTOR ( 1 downto 0 );
  signal estado_proximo : STD_LOGIC_VECTOR ( 1 downto 0 );
  attribute FSM_ENCODED_STATES : string;
  attribute FSM_ENCODED_STATES of \FSM_sequential_estado_atual_reg[0]\ : label is "e2:10,e3:11,e0:00,e1:01";
  attribute FSM_ENCODED_STATES of \FSM_sequential_estado_atual_reg[1]\ : label is "e2:10,e3:11,e0:00,e1:01";
begin
\FSM_sequential_estado_atual[0]_i_1\: unisim.vcomponents.LUT3
    generic map(
      INIT => X"4D"
    )
        port map (
      I0 => Dado0_IBUF,
      I1 => estado_atual(1),
      I2 => estado_atual(0),
      O => estado_proximo(0)
    );
\FSM_sequential_estado_atual[1]_i_1\: unisim.vcomponents.LUT3
    generic map(
      INIT => X"42"
    )
        port map (
      I0 => estado_atual(0),
      I1 => estado_atual(1),
      I2 => Dado0_IBUF,
      O => estado_proximo(1)
    );
\FSM_sequential_estado_atual_reg[0]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => CLK,
      CE => '1',
      CLR => AR(0),
      D => estado_proximo(0),
      Q => estado_atual(0)
    );
\FSM_sequential_estado_atual_reg[1]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => CLK,
      CE => '1',
      CLR => AR(0),
      D => estado_proximo(1),
      Q => estado_atual(1)
    );
S: unisim.vcomponents.LUT3
    generic map(
      INIT => X"80"
    )
        port map (
      I0 => estado_atual(1),
      I1 => estado_atual(0),
      I2 => Dado0_IBUF,
      O => S0_OBUF
    );
end STRUCTURE;
library IEEE;
use IEEE.STD_LOGIC_1164.ALL;
library UNISIM;
use UNISIM.VCOMPONENTS.ALL;
entity FSM2_Matricula_Andre is
  port (
    S1_OBUF : out STD_LOGIC;
    CLK : in STD_LOGIC;
    AR : in STD_LOGIC_VECTOR ( 0 to 0 );
    Dado1_IBUF : in STD_LOGIC
  );
end FSM2_Matricula_Andre;

architecture STRUCTURE of FSM2_Matricula_Andre is
  signal estado_atual : STD_LOGIC_VECTOR ( 1 downto 0 );
  signal estado_proximo : STD_LOGIC_VECTOR ( 1 downto 0 );
  attribute SOFT_HLUTNM : string;
  attribute SOFT_HLUTNM of \FSM_sequential_estado_atual[0]_i_1\ : label is "soft_lutpair1";
  attribute SOFT_HLUTNM of \FSM_sequential_estado_atual[1]_i_1\ : label is "soft_lutpair1";
  attribute FSM_ENCODED_STATES : string;
  attribute FSM_ENCODED_STATES of \FSM_sequential_estado_atual_reg[0]\ : label is "e2:10,e3:11,e0:00,e1:01";
  attribute FSM_ENCODED_STATES of \FSM_sequential_estado_atual_reg[1]\ : label is "e2:10,e3:11,e0:00,e1:01";
begin
\FSM_sequential_estado_atual[0]_i_1\: unisim.vcomponents.LUT3
    generic map(
      INIT => X"9D"
    )
        port map (
      I0 => Dado1_IBUF,
      I1 => estado_atual(1),
      I2 => estado_atual(0),
      O => estado_proximo(0)
    );
\FSM_sequential_estado_atual[1]_i_1\: unisim.vcomponents.LUT3
    generic map(
      INIT => X"60"
    )
        port map (
      I0 => estado_atual(0),
      I1 => estado_atual(1),
      I2 => Dado1_IBUF,
      O => estado_proximo(1)
    );
\FSM_sequential_estado_atual_reg[0]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => CLK,
      CE => '1',
      CLR => AR(0),
      D => estado_proximo(0),
      Q => estado_atual(0)
    );
\FSM_sequential_estado_atual_reg[1]\: unisim.vcomponents.FDCE
    generic map(
      INIT => '0'
    )
        port map (
      C => CLK,
      CE => '1',
      CLR => AR(0),
      D => estado_proximo(1),
      Q => estado_atual(1)
    );
S1_OBUF_inst_i_1: unisim.vcomponents.LUT3
    generic map(
      INIT => X"20"
    )
        port map (
      I0 => estado_atual(1),
      I1 => Dado1_IBUF,
      I2 => estado_atual(0),
      O => S1_OBUF
    );
end STRUCTURE;
library IEEE;
use IEEE.STD_LOGIC_1164.ALL;
library UNISIM;
use UNISIM.VCOMPONENTS.ALL;
entity Top_module is
  port (
    Dado0 : in STD_LOGIC;
    Dado1 : in STD_LOGIC;
    reset : in STD_LOGIC;
    clk : in STD_LOGIC;
    S0 : out STD_LOGIC;
    S1 : out STD_LOGIC
  );
  attribute NotValidForBitStream : boolean;
  attribute NotValidForBitStream of Top_module : entity is true;
  attribute \DesignAttr:ENABLE_AIE_NETLIST_VIEW\ : boolean;
  attribute \DesignAttr:ENABLE_AIE_NETLIST_VIEW\ of Top_module : entity is std.standard.true;
  attribute \DesignAttr:ENABLE_NOC_NETLIST_VIEW\ : boolean;
  attribute \DesignAttr:ENABLE_NOC_NETLIST_VIEW\ of Top_module : entity is std.standard.true;
  attribute ECO_CHECKSUM : string;
  attribute ECO_CHECKSUM of Top_module : entity is "1af67ffb";
end Top_module;

architecture STRUCTURE of Top_module is
  signal Dado0_IBUF : STD_LOGIC;
  signal Dado1_IBUF : STD_LOGIC;
  signal S0_OBUF : STD_LOGIC;
  signal S1_OBUF : STD_LOGIC;
  signal clk_IBUF : STD_LOGIC;
  signal clk_IBUF_BUFG : STD_LOGIC;
  signal reset_IBUF : STD_LOGIC;
  signal sclk05hz : STD_LOGIC;
begin
Dado0_IBUF_inst: unisim.vcomponents.IBUF
     port map (
      I => Dado0,
      O => Dado0_IBUF
    );
Dado1_IBUF_inst: unisim.vcomponents.IBUF
     port map (
      I => Dado1,
      O => Dado1_IBUF
    );
FSM1: entity work.FSM1_Matricula_Otavio
     port map (
      AR(0) => reset_IBUF,
      CLK => sclk05hz,
      Dado0_IBUF => Dado0_IBUF,
      S0_OBUF => S0_OBUF
    );
FSM2: entity work.FSM2_Matricula_Andre
     port map (
      AR(0) => reset_IBUF,
      CLK => sclk05hz,
      Dado1_IBUF => Dado1_IBUF,
      S1_OBUF => S1_OBUF
    );
S0_OBUF_inst: unisim.vcomponents.OBUF
     port map (
      I => S0_OBUF,
      O => S0
    );
S1_OBUF_inst: unisim.vcomponents.OBUF
     port map (
      I => S1_OBUF,
      O => S1
    );
clk_IBUF_BUFG_inst: unisim.vcomponents.BUFG
     port map (
      I => clk_IBUF,
      O => clk_IBUF_BUFG
    );
clk_IBUF_inst: unisim.vcomponents.IBUF
     port map (
      I => clk,
      O => clk_IBUF
    );
clok: entity work.Clock05
     port map (
      AR(0) => reset_IBUF,
      CLK => sclk05hz,
      clockdiv_reg_0 => clk_IBUF_BUFG
    );
reset_IBUF_inst: unisim.vcomponents.IBUF
     port map (
      I => reset,
      O => reset_IBUF
    );
end STRUCTURE;
